  ...
  const handleUpdateAvatar = async () => {
    ...
    try {
      const result = await ImagePicker.launchImageLibraryAsync({
        mediaTypes: ImagePicker.MediaTypeOptions.Images, // Deprecated in newest versions but safe for this Expo version
        allowsEditing: true,
        aspect: [1, 1],
        quality: 0.8,
      });
      ...
      if (!result.canceled && result.assets[0]) {
        ...
        if (uploadRes.data.url) {
          await apiClient.put('/users/me', { avatarUrl: uploadRes.data.url });
          
          // 3. Update local state
          setProfile({ ...profile, avatarUrl: uploadRes.data.url });
        }
      }
    } catch (e) {
